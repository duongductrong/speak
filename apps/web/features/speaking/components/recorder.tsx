...
import { Badge, Button, Card, CardContent, CardFooter, CardHeader, CardTitle, cn } from "@speak/ui";
import { ChevronLeft, ChevronRight, Eye, EyeOff, RefreshCw, Shuffle, Volume2, VolumeX } from "lucide-react";
...
export interface SpeakingRecorderProps extends ComponentProps<"div"> {
  ...
  onPrevious?: () => void;
  onRandom?: () => void;
  onAttemptComplete?: (correctWords: number, totalWords: number, timeSpent: number) => void;
  sessionStats?: {
    accuracy: number;
    totalWords: number;
        ...
        <div className="text-center space-y-6 py-20 animate-fade-in">
          ...
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground">
              Please select a different category or difficulty level from the header.
            </p>
          </div>
        </div>
      </div>
    ...
    <div {...props} className={cn("w-full space-y-6", className)}>
      ...
      {hasStarted && sessionStats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 animate-fade-in">
          ...
          <div className="p-4 rounded-lg bg-muted/50 border">
            <div className="text-xs text-muted-foreground font-[500] mb-1">Words</div>
            <div className="text-2xl font-mono font-[700]">{sessionStats.totalWords}</div>
          </div>

          <div className="p-4 rounded-lg bg-muted/50 border">
            <div className="text-xs text-muted-foreground font-[500] mb-1">Phrases</div>
            <div className="text-2xl font-mono font-[700]">{sessionStats.phrasesCompleted}</div>
          </div>

          <div className="p-4 rounded-lg bg-muted/50 border">
            <div className="text-xs text-muted-foreground font-[500] mb-1">Avg Time</div>
        ...
        <div className="text-center space-y-6 py-12 animate-fade-in">
          <div className="space-y-3">
            ...
            <p className="text-muted-foreground text-lg max-w-md mx-auto">
              Click the microphone button below to start practicing your pronunciation.
            </p>
          </div>

          <div className="flex items-center justify-center gap-2 text-sm text-muted-foreground">
            <Badge className={cn("capitalize", difficultyColors[phrase.difficulty])}>
              {phrase.difficulty}
            </Badge>
            <span>•</span>
            ...
            <span
              className={cn(
                ...
                getWordColorClass(item.status),
                        ...
                        item.status === SPEAKING_STATUS.INCORRECT && "animate-shake"
              )}
            >
              {item.word}
            </span>
        ...
        {hasStarted && !isComplete && (
          <div className="mt-4 space-y-2">
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="font-mono font-[600]">{Math.round(matchPercentage)}%</span>
            </div>
      ...
      {hasStarted && !isComplete && (
        <div className="flex items-center justify-center gap-3 flex-wrap">
          <Button
            ...
            className="gap-2"
          >
            {showPhonetics ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
            Phonetics
          </Button>
          ...
          <Button variant="outline" size="sm" onClick={onRandom} className="gap-2">
      ...
      <div className="text-center text-xs text-muted-foreground space-y-1">
        ...
        <div className="flex items-center justify-center gap-4 flex-wrap">
          <span>
            <kbd className="px-1.5 py-0.5 bg-muted rounded text-[10px] font-mono">Space</kbd> Record
          </span>
          <span>
            <kbd className="px-1.5 py-0.5 bg-muted rounded text-[10px] font-mono">R</kbd> Retry
          </span>
          <span>
            <kbd className="px-1.5 py-0.5 bg-muted rounded text-[10px] font-mono">N / →</kbd> Next
          </span>
          <span>
            <kbd className="px-1.5 py-0.5 bg-muted rounded text-[10px] font-mono">H</kbd> Phonetics
          </span>
        </div>
      </div>
    </div>
